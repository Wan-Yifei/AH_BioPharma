...
write.xlsx(normalizedData, file=paste0(outputPrefix,"-nor.xlsx"))
...
#Sig<-subset(resultsDDS, resultsDDS$pval < 0.05)
#if (nrow(Sig)>0){
...
#pvalueDf<-data.frame(pvalue=resultsDDS$pval)
...
#sigDDS<-subset(resultsDDS, resultsDDS$pval<pval.cutoff & abs(resultsDDS$log2FoldChange)>log2FolderChange.cutoff)
#with(resultsDDS, plot(resultsDDS$log2FoldChange, -log10(resultsDDS$pval), pch=20, main="DE_VolcanoPlot",col="black"))
#with(sigDDS, points(sigDDS$log2FoldChange, -log10(sigDDS$pval), pch=20, col="red"))
#dev.off()
